...
本接口用于在管理端按用户维度浏览“帖子库”数据。数据来源为 `knowledge_base` 表（按用户ID过滤）的记录，文件夹结构复用 `book_folds`（`type = '帖子'`）。当前版本仅提供查看、检索、删除与向量化能力，不支持手工新增/修改。
...
说明：
- 本接口忽略 `type` 参数（代码未读取）。文件夹树请调用 `/api/memo-folders` 并传 `type=posts`。
...
| foldId | ❌ | 指定文件夹过滤：传 `uuid` 仅检索该文件夹；传 `null`/`none` 仅检索未分组；不传表示不过滤 |
