# 备忘录（Memo Notes）API 文档

本接口提供“备忘录”增删改查能力，按文件夹进行组织与浏览。接口风格与项目内现有文档一致（统一 `ApiResponse<T>` 响应格式）。
...
说明：仅返回用于列表所需的精简字段（`id`、`note_name`、`fold_id`、`updated_at`）。
...
响应：`{ success: true, data: { ...更新后的记录(或关键字段)... } }`
